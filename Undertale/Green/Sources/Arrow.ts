...
export class ArrowType {
    ...
    constructor(reverseOdds: number) {
...
export class ArrowSide {
    ...
    constructor(side?: Side, differentSide: boolean = false, reverse: boolean = false) {
        if (side != undefined) 
            this.side = side;
        else {
            let side: Side;
            ...
            do {
                ...
                if (rng < 0.25) side = Side.Up;
                else if (rng < 0.50) side = Side.Right;
                else if (rng < 0.75) side = Side.Down;
                else side = Side.Left;
...
export class ArrowSpeed {
    ...
    constructor(speed?: number, random: number[] = []) {
...
export class Arrow extends CenteredObject {
    ...
    private reverseProg: number = 0;
    private hasReversed: boolean = false;
    
    public rotData: RotationData = new RotationData(0, 0);
    
    constructor(canvas: HTMLCanvasElement, frameCount: number, type: ArrowType, side: ArrowSide, speed: ArrowSpeed, fadeout: boolean = false, dmg: number = 10, iframes: number = 60) {
        let x = 0, y = 0;
        const w = 24, h = 30;
    ...
    public update(game: Game, player: Player, shield: Shield) {
        ...
        // normal behavior
            ...
            switch (this.side) {
                case Side.Up: this.y += this.speed; break;
                case Side.Down: this.y -= this.speed; break;
                case Side.Left: this.x += this.speed; break;
                case Side.Right: this.x -= this.speed; break;
            }
        }
        ...
        switch (this.side) {
            case Side.Up: yHbOffs = -2; break;
            case Side.Down: yHbOffs = 2; break;
            case Side.Left: xHbOffs = -2; break;
            case Side.Right: xHbOffs = 2; break;
        };
    ...
    public distToPlayer(player: Player): number {
        const diff = {
    ...
    private reverseRotate(player: Player) {
    ...
    public static getClosest(player: Player): Arrow | undefined {
        let minDist: number | undefined;
        let closest: Arrow | undefined;
